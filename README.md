...
- **图注召回与裁剪**：扫全库 1900+ 张图的「图N: 标题」图注建语义索引，提问时按相关度召回最匹配的图，用 PyMuPDF 从 PDF 页**裁出图区插入回答**（如问"宇树发展历程"直接附上时间轴图）
- **多轮对话**：checkpointer 按会话保存历史，检索前用 LLM 把含指代/省略的追问改写成独立问题（"它的融资情况呢"→"宇树科技的融资情况"），追问也能精准召回
...
│   └── agent.py         # LangGraph agent：历史改写/路由/拆解/混合检索/精排/改写重试/合成（checkpointer多轮）
└── api/main.py          # FastAPI：SSE流式 /ask（步骤+来源+图表+token）+ /page-image、/figure-image 渲染
web/index.html           # 聊天界面：流式渲染、来源徽章、相关图表区、图表裁剪展示
```

当前语料规模：869 份报告（去重后）→ 32,010 文本页 + 1,481 图表页转写 → 82,981 个向量块；另抽取 18,657 张图注图供按问题召回裁剪。
...
- [v0.7 多轮对话与历史改写](docs/v0.7-多轮对话与历史改写.md) — checkpointer、检索前指代消解、跨轮状态污染坑、为何不能只拼prompt
- [v0.8 语料扩充与规模化评估](docs/v0.8-语料扩充与规模化评估.md) — 120→869份(7×)、增量入库兑现、VL超时/断点续跑/内容审核坑、扩库前后指标对比

## 评估结果（RAGAS，16题分类评估集，DeepSeek裁判）

**基线（v0.1，纯向量top6+直接生成） vs 完整链路（路由/拆解+混合召回+rerank+合成）** —— 在扩库后的 869 份语料上：
...
**扩库前后（完整链路，120份 → 869份语料）** —— 更多语料显著提升检索与生成质量：
...
> 值得一提：v0.4 时完整链路的 Faithfulness 比基线低 0.034（受限于小语料的诚实 trade-off），扩库后翻正为 +0.042。

完整分类别数据见 [eval/results.md](eval/results.md)（扩库前的 120 份结果存档于 [eval/results_120corpus.md](eval/results_120corpus.md)）。
...
- [x] **图注召回与裁剪**：扫全库图注建索引（1900+张图），按问题语义召回最相关的图并从 PDF 裁剪插入回答
- [x] **多轮对话**：LangGraph checkpointer 保存会话历史，检索前历史感知改写（指代消解），追问精准召回
- [x] **语料扩充与规模化评估**：120→869 份（82,981 向量块，7×），增量入库零重编，扩库后三指标全面提升
